    ...
    return(
        <section className = {` bg-[url(./imgs/mainSection.jpg)] h-[630px] sm:h-[750px] w-full bg-center bg-no-repeat bg-cover flex flex-col items-center`}>
            ...
            <div className=" flex flex-col md:flex-row w-[70%] md:w-[80%]  justify-around">
                <Button className="rounded-2xl m-3"><Link to={"/registro"} className=" text-white text-lg sm:text-xl md:text-2xl uppercase">aplica por primera vez</Link></Button>
                <Button className="rounded-2xl m-3"><Link to={"/actualiza-perfil"} className=" text-white text-lg sm:text-xl md:text-2xl uppercase">actualiza tu perfil</Link></Button>
            </div>
        </section>
    )
}
